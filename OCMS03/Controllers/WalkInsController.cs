...
namespace OCMS03.Controllers
{
    public class WalkInsController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> MakeAppointment(RegisterWalkInsViewModel model, string id)
        {
            PopulateWalkInsDropDownList();
            PopulateClinicDropDownList();
            if (ModelState.IsValid)
            {
                try
                {
                   ...
                   isAvailable = await CheckAvailabilityAsync(
                   model.NurseId,
                   model.AppointmentDate,
                   model.StartTime);
                    ...
                    var walkinApp = new WalkIn_Appointment
                    {
                    ...
                    return RedirectToAction(nameof(MakeAppointment));
                }
                catch (Exception)
                {
